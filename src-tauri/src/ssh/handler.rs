    ...
    async fn check_server_key(&mut self, key: &ssh_key::PublicKey) -> Result<bool, Self::Error> {
        match check_known_hosts_path(&self.hostname, self.port, key, &self.known_hosts_path) {
            ...
            Err(russh::keys::Error::KeyChanged { .. }) => {
                self.ui.emit_host_key_mismatch(&self.prompt_payload(key));
                Ok(false)
            }
...
impl ClientHandler {
    fn prompt_payload(&self, key: &ssh_key::PublicKey) -> HostKeyPrompt {
        HostKeyPrompt {
    ...
    async fn prompt_unknown_key(&self, key: &ssh_key::PublicKey) -> Result<bool, russh::Error> {
        ...
        let accepted = self.ui.ask_host_key(self.prompt_payload(key)).await;
        if accepted {
